...
const Pedido = ({pedido})=>{
    ...
    //Mutation para cambiar el estado de un pedid
    ...
    const confirmarEliminarPedido = () =>{
        Swal.fire({
            title: 'Esta Seguro que deseas eliminar este pedido?',
    ...
    return (
       <div className={`${clase} border-t-4  mt-4 bg-white rouded p-6 md:grid md:grid-cols-2 md:gap-4 shadow-lg`}>
           <div>
           ...
           <div>
                ...
                {pedido.pedido.map(articulo =>(
                    <div key={articulo.id} classNames="mt-4">
                        <p classNames="text-sm text-gray-600">Producto: {articulo.nombre}</p>
                        <p classNames="text-sm text-gray-600">Cantidad: {articulo.cantidad}</p>  
                    </div>
                ))}
